<template>
    <article class="nav">
      <nav :class="['menu', { active: isMenuActive }]" :aria-hidden="!isMenuActive">
        <ul>
          <li v-for="(item, index) in menuItems" :key="item.href" :class="{ active: index === 0 }">
            <a :href="item.href" :tabindex="isMenuActive ? '0' : '-1'">
    ...
    <button
      class="btn"
      :class="{ close: isMenuActive, open: !isMenuActive }"
      @click="toggleMenu"
